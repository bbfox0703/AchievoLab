...
public class ImageFailureTrackingServiceTests
{
    ...
    public void ExponentialBackoff_WorksCorrectly()
    {
        ...
        try
        {
            ...
            Assert.True(tracker.ShouldSkipDownload(appId, "japanese")); // 8 分鐘前失敗，還在 10 分鐘退避期內
            ...
            Assert.False(tracker.ShouldSkipDownload(appId, "japanese")); // 11 分鐘前失敗，已超過 10 分鐘

            // Second failure - should skip for 20 minutes (failure count = 2)
            tracker.RecordFailedDownload(appId, "japanese", "TestGame2");
            Assert.True(tracker.ShouldSkipDownload(appId, "japanese")); // 剛失敗，在 20 分鐘退避期內
            ...
            Assert.False(tracker.ShouldSkipDownload(appId, "japanese")); // 25 分鐘前失敗，已超過 20 分鐘

            // Third failure - should skip for 40 minutes (failure count = 3)
            tracker.RecordFailedDownload(appId, "japanese", "TestGame2");
            Assert.True(tracker.ShouldSkipDownload(appId, "japanese")); // 剛失敗，在 40 分鐘退避期內
        }
        finally
        {
            Directory.Delete(tempDir, true);
